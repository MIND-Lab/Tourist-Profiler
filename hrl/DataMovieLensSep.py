...
class Dataset(object):
    def __init__(self, data_path, num_negatives, batch_size, fast_running=False):
        ...
        negative_file_path = f"{self.data_path}/preprocessed/test_negatives_sep.csv"
    ...
    def load_dataset_as_dataframe(self):
        ...
        data_tr, data_val, data_te = self._split_train_test(data_full)
    ...
    def _get_interacted_items_list(self, data):
        ...
        # If we don't have a specific user in the set, we don't add anything. 
        data_grouped_by_user = data.groupby('UserID')
        lists = list()
        for _, group in data_grouped_by_user:
            group = group.sort_values(by=['Timestamp', 'MovieID'])
    ...
    def load_separated_dataset_as_dataframe(self):
        ...
        data_full_sep.to_csv(f"{self.data_path}/preprocessed/data_full_sep.csv", index=False)
        
        user_id_train_mapping = {old_user_id: new_user_id for new_user_id, old_user_id in enumerate(data_tr_sep['UserID'].unique())}
        data_tr_sep['UserID'] = data_tr_sep['UserID'].map(user_id_train_mapping)
        
        user_id_test_mapping = {old_user_id: new_user_id for new_user_id, old_user_id in enumerate(data_te_sep['UserID'].unique())}
        data_te_sep['UserID'] = data_te_sep['UserID'].map(user_id_test_mapping)
    ...
    def _split_train_val_test_separated(self, data, test_proportion=0.3, val_proportion=0.5):
        ...
        # We save the train, validation and test sets in CSV files.
        data_tr_sep.to_csv(f"{self.data_path}/preprocessed/data_tr_sep.csv", index=False)
        data_val_sep.to_csv(f"{self.data_path}/preprocessed/data_val_sep.csv", index=False)
        data_te_sep.to_csv(f"{self.data_path}/preprocessed/data_te_sep.csv", index=False)
    ...
    def load_training_file_as_matrix(self):
        data_tr = self.data_tr_sep
        data_tr['BinaryRating'] = (data_tr['Rating'] > 0).astype(int)
        self.num_train_users = len(pd.unique(self.data_tr_sep['UserID']))
        ...
        non_zero_rows_indices = mat.sum(axis=1).nonzero()[0]
        mat = mat[non_zero_rows_indices]
        return mat
    
    
    def load_training_file_as_list(self):
        return self.lists_tr_sep
    
    
    def load_rating_file_as_list(self):
        data_te = self.data_te_sep
        testRatings = []
    ...
    def save_negative_file(self):
        ...
        for idx in tqdm(range(len(self.testRatings))):
            ...
            all_movies = set(data_te['MovieID'].unique())
        ...
        testNegativesDataframe.to_csv(f"{self.data_path}/preprocessed/test_negatives_sep.csv", index=False)    


    def load_negative_file(self):
        testNegativesDataframe = pd.read_csv(f"{self.data_path}/preprocessed/test_negatives_sep.csv")
