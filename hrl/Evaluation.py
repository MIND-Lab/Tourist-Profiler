...
def predict_with_attentions(model, test_user_input, test_num_idx, test_item_input, test_labels, test_batch_num):
    ...
    for batch in range(test_batch_num):
        ...
        predictions, attention, loss = model.predict_with_attentions(t_u, np.reshape(t_num,(-1,1)), np.reshape(t_i,(-1,1)), np.reshape(t_label,(-1,1)))
